    ...
    def load(self, filename, e=True, d=False):
        ...
        if e:
            try:
                ...
                print(f'[Load] Warning: Strict loading failed, trying with strict=False')
                print(f'       Error: {err}')
                self.model.load_state_dict(state['model_state'], strict=False)
            print('Load Encoder successfully from [%s]' % filename)
        if d:
            try:
                self.d_classifier.load_state_dict(state['discriminator'], strict=True)
            except RuntimeError as err:
                print(f'[Load] Warning: Strict loading failed for discriminator, trying with strict=False')
                print(f'       Error: {err}')
                self.d_classifier.load_state_dict(state['discriminator'], strict=False)
