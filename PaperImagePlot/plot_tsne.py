...
TSNE_FIG_PATH = r'C:\Users\CQU\Desktop\DASMN_revised_2020_11\fig\DASMN\Discussion_DA'
...
class DASMNLearner:
    ...
    def plot_adaptation(self, x_s, x_t):
        ...
        labels = ['NC-s', 'OF-s', 'RoF-s','IF-s', 'NC-t', 'OF-t', 'RoF-t', 'IF-t']
        tSNE_fun(x.cpu().detach().numpy(), shot=self.ns, name=None, labels=labels, n_dim=2)
...
def train_operate(way, ns, nq, save_path, final_test=True, load_path=None):
    ...
    #                                   normalize=True, data_len=DIM, label=False, tgt_set='ims')
    # _, tar = generator.IMS_3way(examples=100, split=0, way=way, normalize=True,
    #                                    label=False, data_len=DIM)

    # src, _ = generator.IMS_3way(examples=100, split=running_params['train_split'], way=way, normalize=True,
    #                                    label=False, data_len=DIM)
    # _, tar = generator.CQU_4way(examples=100, split=0, way=way, normalize=True,
    #                                    label=False, data_len=DIM)

    src, _ = generator.IMS_3way(examples=100, split=running_params['train_split'], way=way, normalize=True,
                                      label=False, data_len=DIM)
    _, tar = generator.CW_cross(way=way, examples=100, split=0, data_len=DIM,
                                       normalize=True, label=False, tgt_set='ims')

    # src, _ = generator.CQU_4way(examples=100, split=running_params['train_split'], way=way, normalize=True,
    #                                    label=False, data_len=DIM)
    # _, tar = generator.IMS_3way(examples=100, split=0, way=way, normalize=True,
    #                                    label=False, data_len=DIM)
#####################################################################################################################################
    ...
    if final_test:
        ...
        nets.test(tar_tasks=tar, src_tasks=None, save_fig_path= save_path, model_eval=True)
        nets.test(tar_tasks=tar, src_tasks=None, save_fig_path= save_path, model_eval=False)
...
def test_operate(way, ns, nq, path, eval_stats, fig_path, ob_domain=False, num_domain=30):
    ...
    src_tasks, _ = generator.CW_cross(way=way, examples=100, split=running_params['train_split'],
                                      normalize=True, data_len=DIM, label=False, tgt_set='ims')
    _, test_tasks = generator.CQU_4way(examples=100, split=0, way=way, normalize=True,
                                       label=False, data_len=DIM)

    # src_tasks, _ = generator.CQU_4way(examples=100, split=running_params['train_split'], way=way, normalize=True,
    #                                    label=False, data_len=DIM)
    # _, test_tasks = generator.IMS_3way(examples=100, split=0, way=way, normalize=True,
    #                                    label=False, data_len=DIM)

    # src_tasks, _ = generator.IMS_3way(examples=100, split=running_params['train_split'], way=way, normalize=True,
    #                                    label=False, data_len=DIM)
    # _, test_tasks = generator.CQU_4way(examples=100, split=0, way=way, normalize=True,
    #                                    label=False, data_len=DIM)
...
if __name__ == "__main__":
    ...
    m_f_root = r"C:\Users\CQU\Desktop\DASMN_revised_2020_11\Modelsave\CW10\DASMN"

    # train:
    flag = input('Train? y/n\n')
    if flag == 'y' or flag == 'Y':
        save_path = os.path.join(m_f_root, 'dasmn_SACL_' + str(running_params['train_split']))
    ...
    flag = input('Test? y/n\n')
    if flag == 'y' or flag == 'Y':
        load_path = r"C:\Users\CQU\Desktop\DASMN_revised_2020_11\Modelsave\CW10\DASMN\dasmn_funnySAnCLtest_toCQU_100\final_epoch10"
        fig_path = r"C:\Users\CQU\Desktop\DASMN_revised_2020_11\fig\DASMN\DASMN_Csa30_2.svg"
